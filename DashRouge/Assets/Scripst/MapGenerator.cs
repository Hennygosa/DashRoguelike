...
public class MapGenerator : MonoBehaviour
{
    ...
    void RandomFillMap()
    {
        ...
        for (int x = 0; x < width; x++) 
        {
            for (int y = 0; y < height; y++) 
            {
                //border always a wall
                if (x == 0 || x == width-1 || y == 0 || y == height-1)
                {
                    map[x, y] = 1;
                }
                //center bottom - player spawn point
                else if (x >= width / 2 - 5 && x <= width / 2 + 5 && y > 0 && y < 10)
                {
                    map[x, y] = 0;
                }
    ...
    {
        int wallCount = 0;
        //cycle 3x3 around the specific tile
